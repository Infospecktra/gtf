...
public class ScheduledProjectListBuilder implements WebControlBuilder
{
   ...
   private NavigationTree createTree(ScheduledProjectListBean projectsBean, String codeBase)
   {
      ...
      projectsRoot.setActionLink("/wf/projectInfo.wf?actiontype=clear&targetPage=application");
      ...
      try
      {
         ...
         String pid = null;
         NavigationNode timetableNode = null;
         String tid = null;
         ...
         for(int i=0; i<projectsSize; i++)
         {
            ...
            projectNode.setActionLink("/wf/projectInfo.wf?actiontype=load&id="+pid+"&targetPage=application");
            ...
            timetables = projectsBean.getTimetableUnderProject(projects[i]);
            int timetablesSize = timetables.length;
            for(int k=0; k<timetables.length; k++)
            {
               tid = timetables[k].getId();
               String timetableNodeId = projectNodeID + "." + tid;
               timetableNode = NavigationNode.createNewNode(NavigationNode.NODE_STYLE);
               timetableNode.setId(timetableNodeId);
               System.out.println("------>>" + timetables[k].getProjectType());
               timetableNode.setCaption(timetables[k].getProjectType());
               timetableNode.setActionLink("/wf/timetable.wf?actiontype=load&view=timetable&id=" + tid + "&targetPage=application");
               timetableNode.setNavigationActionLink("scheduledProjectList.wf");
               timetableNode.setIconOpen(projectIcon);
               timetableNode.setIconClose(projectIcon);
               // the last node of this level
               timetableNode.setCodeBase(codeBase);
               if(i==timetablesSize-1)
                  timetableNode.setIsTheLastNode(true);
               timetableNode.setLevel(3);

               timetableNode.setParent(projectNode);
               projectNode.addChildNode(timetableNode);

               Schedule[] schedules = projectsBean.getSchedulesUnderTimetable(timetables[k].getId());
               for(int j=0; j<schedules.length; j++)
               {
                  sid = schedules[j].getId();
                  String stageNodeID = timetableNodeId + "." + sid;
                  ...
                  if(null!=(date=schedules[j].getDueDate()))
                     stageNode.setCaption(schedules[j].getDisplayName()+"[" + date.toString().replace('-', '/') + "]");
                  else
                     stageNode.setCaption(schedules[j].getName());
                  //stageNode.setActionLink("/wf/timetable.wf?actiontype=load&view=schedule&id=" + pid + "&name=" + schedules[j].getName());
                  ...
                  stageNode.setLevel(4);
                  // the last node of this level
                  if(j==schedules.length-1)
                     stageNode.setIsTheLastNode(true);

                  stageNode.setParent(timetableNode);
                  timetableNode.addChildNode(stageNode);
               }
            }
         }
      }
