...
public class ProjectNotesFormBuilder implements WebControlBuilder
{
   public WebControl build(GenericBean bean) throws Exception
   {
      ReportsStageAccessBean projectBean = (ReportsStageAccessBean)bean;
      String codeBase = projectBean.getAppBase() + projectBean.getGuiBase();
      Project currentProject = projectBean.getCurrentProject();
      boolean isClient = projectBean.gotPermit("ReportManager", projectBean.getProjectType(), ReportManager.CLIENT);
...
/*
      ...
      for(int i=0; i<dataGroups.length; i++)
      {
         ...
         {
            data = new TextField();
            data.setTitle(datas[j].getDisplayName());
            data.setId("datas_" + datas[j].getName());
            data.setName("datas_" + datas[j].getName());
            data.setValue(datas[j].getValue());
            data.setUnit(datas[j].getUnit());
            data.setSize("25");
            data.setDisplayOnly(false);
            data.setIsMandatory(datas[j].getIsRequired());
      ...
      HashMap stages = projectBean.getCurrentAvailableStages(currentProject.getId());
      if(null!=stages&&0<stages.size())
      {
         ...
         templates = projectBean.getAllStageTemplates();
