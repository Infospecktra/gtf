  /*
	...
	*/
	...
	//*------------------------------------------------------------------
	function redrawTree() {
		var doc = eval(FT_treeTarget + '.' + 'document'); //** FT_treeTarget is a globle val
		doc.clear();
		drawDocHeader(doc);
	...
	function redrawNode(foldersNode, doc, level, lastNode, leftSide) {
		...
		if (level>0) {
			...
			{
				if (foldersNode[2]==1) {
					doc.write('<img src="'+FT_IMG_lastnode+'" width="16" height="22">');
				}
				else {

					if (foldersNode[1]) {
						var ss = '<a href="javascript:' + FT_treeTargetParent + '.openBranch(\'onlyexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')" onMouseOver="window.status=\'Close folder\'; return true">';
						doc.write(ss + '<img src="'+FT_IMG_lastnode_cls+'" border="0"></a>');
					}
					else {
					   var ss = '<a href="javascript:' + FT_treeTargetParent + '.openBranch(\'onlyexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')" onMouseOver="window.status=\'Open folder\'; return true">';
					   doc.write(ss + '<img src="'+FT_IMG_lastnode_opn+'" border="0"></a>');
					}
				}
				leftSide = leftSide + '<img src="'+FT_IMG_blank+'" width="16" height="22">';
			}
			else
			{
				if (foldersNode[2]==1) {
					doc.write('<img src="'+FT_IMG_node+'" width="16" height="22">');
				}
				else {
					if (foldersNode[1]) {
						var ss = '<a href="javascript:' + FT_treeTargetParent + '.openBranch(\'onlyexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')" onMouseOver="window.status=\'Close folder\'; return true">';
						doc.write(ss + '<img src="'+FT_IMG_node_cls+'" border="0"></a>');
					}
					else {
					   var ss = '<a href="javascript:' + FT_treeTargetParent + '.openBranch(\'onlyexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')" onMouseOver="window.status=\'Open folder\'; return true">';
					   doc.write(ss + '<img src="'+FT_IMG_node_opn+'" border="0"></a>');
					}
				}
				leftSide = leftSide + '<img src="'+FT_IMG_vertline+'" width="16" height="22">';
			}
		}
	...
	function displayLeafNode(docNode, doc) {
		...
		if (docNode[4]!='') { //** will not generate link if there is no link
			linkString = '<a href="' + docNode[4] + '" target="' + FT_allLinkTarget + '" ' + docNode[5] +' onClick="javascript:' + FT_treeTargetParent + '.openBranch(\'unexpand\',' + docNode[0] + ',\'' + repFirstSQ(docNode[3]) + '\')">'; //** FT_allLinkTarget is a globle val
		}
		...
		if (foldersNode[2]==1) { //** leaf node
			...
			if (foldersNode[0]==-1) { //* special case: base node
				...
				if (foldersNode[4]=='')
					...
					doc.write('<a href="' + foldersNode[4] + '" target="' + FT_allLinkTarget + '" ' + foldersNode[5] + ' onClick="javascript:'+FT_treeTargetParent+'.openBranch(\'unexpand\','+foldersNode[0]+',\''+repFirstSQ(foldersNode[3])+'\')"><span class="BASESTYL">'+(onNode?'<span class="ONNODESTYL">':'')+foldersNode[3]+(onNode?'</span>':'')+'</span></a>');
			}
			else {
				var rootNode = false;
				var tempstr = '';
				...
				if (foldersNode[1]) {
					linkString = '<a href="javascript:' + FT_treeTargetParent + '.openBranch(\'onlyexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')" onMouseOver="window.status=\'Close folder\'; return true">';
					doc.write(linkString + '<img src="' + foldersNode[7] + '" border="0"></a>');
				}
				else {
				   linkString = '<a href="javascript:' + FT_treeTargetParent + '.openBranch(\'onlyexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')" onMouseOver="window.status=\'Open folder\'; return true">';
				...
				if (!foldersNode[1]) {
					tempstr= FT_treeTargetParent + '.openBranch(\'allexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')';
				}
				else {
					tempstr= FT_treeTargetParent + '.openBranch(\'unexpand\',' + foldersNode[0] + ',\'' + repFirstSQ(foldersNode[3]) + '\')';
				}
				//** case 2: click on the Link String, the node will close/open
				/*
					tempstr=' onClick="javascript:' + FT_treeTargetParent + '.openBranch(\'allexpand\',' + foldersNode[0] + ',\'' + foldersNode[3] + '\')" ';
				*/

				if (foldersNode[4]!='') { // has link, open/close folder based on case 1 or 2, and go to link in the right frame
					linkString = '<a href="' + foldersNode[4] + '" target="' + FT_allLinkTarget + '" ' + foldersNode[5] + ' onClick="' + tempstr + '">';
				}
				else {
					//** no link, open or close folder only
					//linkString = '<a href="javascript:'+FT_treeTargetParent + '.openBranch(\'allexpand\',' + foldersNode[0] + ',\'' + foldersNode[3] + '\')'+'">';
	...
	*/
	...
	//*** this method should be developed into full replace "'" functionality...
	//*** it only replaces the first "'" currently, for special purpose
	function repFirstSQ(inStr) {
		var v = inStr.indexOf("'");
		if (v == -1) {
			return inStr;
		}
		else {
			return ( inStr.substring(0,inStr.indexOf("'")) + "\\'" + inStr.substring(inStr.indexOf("'")+1,inStr.length-1) );
		}
	}
	...
	function openBranchName(branchName) {
		...
		clickOnFolderRecName(FT_mybase, branchName);
	...
	function drawDocHeader(doc) {
		...
		//doc.writeln('a:hover '+FT_aHoverCCS);

		doc.writeln('.BASESTYL ' + FT_baseNodeCCS);
		doc.writeln('.ROOTSTYL ' + FT_rootNodeCCS);
		doc.writeln('.NODESTYL ' + FT_folderNodeCCS);
		doc.writeln('.LEAFSTYL ' + FT_docNodeCCS);
		doc.writeln('.ONNODESTYL ' + FT_onNodeCCS);

		doc.writeln('</style>');

		doc.writeln('<link rel=stylesheet type="text/css" href="'+FT_linkedCCS+'">');

		doc.writeln('</head>');
		doc.writeln('<body '+FT_treeBgProperty+'>');

		//doc.writeln('<div id="overDiv" style="position:absolute; visibility:hide; z-index:999"></div>');
		//doc.writeln("<scr" + "ipt language='JavaScript' src='../../js/overlib.js'></scr" + "ipt>");
	}
	...
	function checkAllClickOnRec(id, branchName) {
		...
		clickOnFolderRec(FT_mybase, id, branchName);
